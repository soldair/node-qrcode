...
test('QRCode error correction', function (t) {
  ...
  for (let l = 0; l < ecValues.length; l++) {
    for (let i = 0; i < ecValues[l].name.length; i++) {
      ...
      t.deepEqual(qr.errorCorrectionLevel, ecValues[l].level, 'Should have correct errorCorrectionLevel value')
      t.notThrow(function () {
...
test('QRCode version', function (t) {
  ...
  t.throw(function () {
    qr = QRCode.create(new Array(Version.getCapacity(2, ECLevel.H)).join('a'), { version: 1, errorCorrectionLevel: ECLevel.H })
  }, 'Should throw if data cannot be contained with chosen version')
  t.throw(function () {
    qr = QRCode.create(new Array(Version.getCapacity(40, ECLevel.H) + 2).join('a'), { version: 40, errorCorrectionLevel: ECLevel.H })
