...
test('Version capacity', function (t) {
  ...
  for (let l = 0; l < EC_LEVELS.length; l++) {
    for (let i = 1; i <= 40; i++) {
      t.equal(Version.getCapacity(i, EC_LEVELS[l], Mode.NUMERIC), EXPECTED_NUMERIC_CAPACITY[i - 1][l], 'Should return correct numeric mode capacity')
      t.equal(Version.getCapacity(i, EC_LEVELS[l], Mode.ALPHANUMERIC), EXPECTED_ALPHANUMERIC_CAPACITY[i - 1][l], 'Should return correct alphanumeric mode capacity')
      t.equal(Version.getCapacity(i, EC_LEVELS[l], Mode.KANJI), EXPECTED_KANJI_CAPACITY[i - 1][l], 'Should return correct kanji mode capacity')
      t.equal(Version.getCapacity(i, EC_LEVELS[l], Mode.BYTE), EXPECTED_BYTE_CAPACITY[i - 1][l], 'Should return correct byte mode capacity')
      t.equal(Version.getCapacity(i, EC_LEVELS[l]), EXPECTED_BYTE_CAPACITY[i - 1][l], 'Should return correct byte mode capacity')
    }
  }
  t.end()
})
test('Version best match', function (t) {
  function testBestVersionForCapacity (expectedCapacity, DataCtor) {
    for (let v = 0; v < 40; v++) {
      for (let l = 0; l < EC_LEVELS.length; l++) {
        ...
        t.equal(Version.getBestVersionForData([data], EC_LEVELS[l]), v + 1, 'Should return best version')
        if (l === 1) {
          t.equal(Version.getBestVersionForData(data, null), v + 1, 'Should return best version for ECLevel.M if error level is undefined')
          t.equal(Version.getBestVersionForData([data], null), v + 1, 'Should return best version for ECLevel.M if error level is undefined')
        }
      }
    }
    for (let i = 0; i < EC_LEVELS.length; i++) {
      ...
      t.notOk(Version.getBestVersionForData(tooBigData, EC_LEVELS[i]), 'Should return undefined if data is too big')
      t.notOk(Version.getBestVersionForData([tooBigData], EC_LEVELS[i]), 'Should return undefined if data is too big')
      t.notOk(Version.getBestVersionForData(tooBigDataArray, EC_LEVELS[i]), 'Should return undefined if data is too big')
    }
  }
  ...
  t.ok(Version.getBestVersionForData([new ByteData('abc'), new NumericData('1234')]), 'Should return a version number if input array is valid')
