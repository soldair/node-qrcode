...
test('Polynomial', function (t) {
  ...
  result = Poly.mod(p1, p2)
  t.ok(Buffer.isBuffer(result), 'Should return a buffer')
  t.equal(result.length, 4, 'Should return correct number of coefficients')
...
test('Generator polynomial', function (t) {
  ...
  t.ok(Buffer.isBuffer(result), 'Should return a buffer')
  t.deepEqual(result, [1], 'Should return coeff [1] for polynomial of degree 0')
