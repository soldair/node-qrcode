...
exports.render = function render (qrData, options) {
  ...
  var scale = Math.floor(Utils.getScale(size, opts))
  var qrcodesize = Utils.getImageWidth(size, opts)
  ...
  xmlStr += '<defs><rect id="p" width="' + scale + '" height="' + scale + '" /></defs>\n'
  ...
  for (var i = 0; i < size; i++) {
    for (var j = 0; j < size; j++) {
      ...
      var x = (opts.margin + j) * scale
      var y = (opts.margin + i) * scale
