...
exports.render = function render (qrData, options) {
  ...
  xmlStr = data.reduce(function (xml, bit, index) {
    if (!bit) return xml

    var x = (opts.margin + (index % size)) * opts.scale
    var y = (opts.margin + Math.floor(index / size)) * opts.scale
...
exports.renderToFile = function renderToFile (path, qrData, options) {
  ...
  return fs.writeFileSync(path, svg)
}
